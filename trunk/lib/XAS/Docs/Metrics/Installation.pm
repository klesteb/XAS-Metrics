package XAS::Docs::Metrics::Installation;
...
XAS::Docs::Metrics::Installation - how to install the XAS Metrics processor
...
Since the code repository is git based, you can use the following commands:
    ...
    # cd XAS-Metrics
    # git init
    # git pull http://scm.kesteb.us/git/XAS-Metrics master

Or you can download the code from CPAN in the following manner:

    # cpan -g XAS-Metrics
    # tar -xvf XAS-Metrics-0.01.tar.gz
    # cd XAS-Metrics-0.01
...
Debian build environment, then you can do the following:
    ...
    # debian/rules build
    # debian/rules clean
    # dpkg -i ../libxas-metrics-perl_0.01-1_all.deb
...
rpm build environment, then you can do the following:
    ...
    # ./Build test
    # ./Build dist
    # rpmbuild -ta XAS-Metrics-0.01.tar.gz
    # cd ~/rpmbuild/RPMS/noarch
    # yum --nogpgcheck localinstall perl-XAS-Metrics-0.01-1.noarch.rpm
...
On Unix/Linux do the following:
    ...
    # xas-create-schema --schema XAS::Model::Database::Metrics --directory .
    # sqlite3 /var/lib/xas/metrics.db < ./XAS-Model-Schema-0.01-SQLite.sql
    # rm -f ./XAS-Model-Schema-0.01-SQLite.sql

On Windows do the following:

    > xas-create-schema --schema XAS::Model::Database::Metrics --directory .
    > sqlite3 \XAS\lib\metrics.db < XAS-Model-Schema-0.01-SQLite.sql
    > delete XAS-Model-Schema-0.01-SQLite.sql

=head1 POST INSTALLATION

You should check the metric configuration file to see if the proper
modules are being loaded. 

Once that is done. You need to start the collector. On Debian or RHEL
you would issue the following commands:

    # service xas-metrics start
    # chkconfig --add xas-metrics

On Windows, use these commands:

   > xas-metrics --install
   > sc start XAS_METRICS
...
The latest and greatest is always available at:
    ...
    http://scm.kesteb.us/git/XAS-Metrics
